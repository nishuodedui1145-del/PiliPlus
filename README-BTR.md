...
| **sidx 分段索引规划** | 起播后台预取 256 KiB 解析 init+sidx，**按媒体分段边界对齐**切分（片内仍按 512 KiB 细分，防止单片过大撑爆在途内存）；seek / 切流复用缓存，不重拉索引 |
| **跨节点总长度一致性校验** | 不同节点返回的文件总长度不一致即终止换路线，**绝不混拼** |
| **降级直连 + 自动重接管** | 加速不可行时 3.5 s 宽限后降级为直连透传，再按 4 s / 8 s / 16 s 自动重接管最多 3 次（2 分钟窗口重置） |
| **节点封禁三级粒度** | host / 具体地址 / 二者组合三级 + 速度 EWMA 平滑 + 指数退避（上限 60 s）；**全部被封时仍继续尝试**（fail-open，绝不返回空候选） |
...
| **音视频分开预算** | 画面 / 声音各自分配连接额度，避免音频抢画面的并发 |
...
| 上游的 sidx **分段边界不能直接当传输片长** | B 站真实分段单片就有 5~8 MiB → 一段一片时在途内存 ≈ 并发 × 单片（实测 33 × ~7 MiB ≈ **230 MB**），并让「慢块阈值」判据成片误报 → 正解是**对齐分段起点 + 片内 512 KiB 细分** |
| 上游里的「**等 N 秒**」必须按宿主翻译 | 官方 3500 ms 是「失败后把视频交回原生播放器」前的等待；照搬进本地代理的请求路径就是**真的卡 3.5 秒** → 延迟一律做成时间戳 |
| 并发**不能创造不存在的国际带宽** | 官方原话；实测 4K（20.2Mbps）在差线路上聚合只有 0.5~2 MB/s，而同一线路单连接瞬时能到 2.14 MB/s |

## 测试

```bash
python tool/make_btr_mirror.py           # 把 lib/services/btr_proxy/ 镜像成独立测试台（改了模块必须重跑）
...
flutter test --no-pub test/standalone/btr_round22_test.dart        # sidx 规划 / 总长一致性 / 降级重接管 / 三级封禁 / 预算
...
- 多连接治的是「单条连接被限速」；**治不了「首次回源」**（冷门内容海外节点无缓存，第一遍要走最差的国际链路）—— 冷门视频首次卡顿，重新加载一次通常就好。
...
- 并发下载的**原理与参数**参考自 **Bilibili-thread-ripper**（网页版与桌面版，**MIT** 许可）：
  ...
  - 本移植为**独立实现**（Dart，本地代理架构），未直接复制其 JS 代码；并发的爬坡/切换/竞速策略是本移植自研，其余默认值（并发 8、hedge 延迟、起播缓冲档位、失败兜底节奏等）参照其源码。
